...
def merge(path_to_main_tree, path_to_subtree, keyword, verbose = False, check = False):
    console = Console()
    if verbose == True:
        console.print(Text.assemble("Provided filepath for maintreee ", (f"{path_to_main_tree}", "underline"), style = "italic"))
    maintree = {}
    ...
    if range(len(maintree)) != 1:
        ...
        try:
            ...
            if check == True:
                ...
                console.print(Text.assemble("As the 'check' option was not indicated, the provided trees will not be verified. Please know that it can lead to errors in the process of merging."), style = error)
                mergetrees = {}
                mergetrees = {key:[] for key in range(len(subtrees))}
                for i in range(len(subtrees)):
                    mergetrees[i] = tree_obj()
